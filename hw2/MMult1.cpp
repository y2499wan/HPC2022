...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  #endif 
  // #pragma omp schedule(static,4)
  for (long j = 0; j < n; j+=BLOCK_SIZE) { 
    for (long p = 0; p < k; p+=BLOCK_SIZE) { 
      for (long i = 0; i < m; i+=BLOCK_SIZE ) {
        for (long blkcol = j; blkcol < j + BLOCK_SIZE; blkcol++) {
          for (long pp = p; pp < p + BLOCK_SIZE; pp++) {
            for (long blkrow = i; blkrow < i + BLOCK_SIZE; blkrow++) {
              double A_ip = a[pp*m+blkrow];
              double B_pj = b[blkcol*k+pp];
              //double C_ij = c[blkcol*m+blkrow];
              double C_ij = A_ip * B_pj;
              //#pragma omp atomic update
              #ifdef _OPENMP
                #pragma atom update
              #endif
              c[blkcol*m+blkrow] += C_ij;
            } 
          }
        }
      }
