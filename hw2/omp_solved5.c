...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      omp_set_lock(&locka);
      // lock/unlock individual mutex objects in different orders will cause deadlock. Consider section 1
      // lock a, then section 2 lock b, then section1 lock a, and then section2 lock b. The deadlock 
      // occurs. The solution is to ensure the lock orders are consistent in two sections.
      for (i=0; i<N; i++)
        b[i] = i * PI;
      omp_set_lock(&lockb);
