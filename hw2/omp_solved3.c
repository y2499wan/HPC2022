...
void print_results(float array[N], int tid, int section) 
{
  ...
  //#pragma omp barrier
  //* two threads used for computations will wait forever here for the other two threads reach the barrier
  // the other two thread will never reach this barrier as they are not responsible for sections
  // so we should remove the barrier.
